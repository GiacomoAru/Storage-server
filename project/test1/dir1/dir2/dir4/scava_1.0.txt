...
--incrementa e decrementa di uno il paramentro
local incr = function(a) 
	return a + 1;
end
local decr = function(a)
	return a - 1;
end
...
local dig = function(where)
	...
	blocchiScavati = incr(blocchiScavati);
end
...
local digAndMove = function(where)
	...
	if where == "forward" then if turtle.inspect() then intelligentDig("forward") end; turtle.forward()
		elseif where == "up" then if turtle.inspectUp() then intelligentDig("up") end; turtle.up()
			elseif where == "down" then if turtle.inspectDown() then intelligentDig("down") end;turtle.down() end
	
	passiTotali = incr(passiTotali);
end 
...
local checkAndMine = function()
	...
	repeat
		...
		if(ifb and whatb.tags["forge:ores"]) then mineraliScavati = incr(mineraliScavati) end
		attBlock = incr(attBlock);
...
local start = function(pattern, nTunnel, dTunnel, lato, tunnelDone) 
	...
	while tunnelDone < nTunnel do
		...
		tunnelDone = incr(tunnelDone);
		passiFatti = 0;
	end
	
	print("Scavo... :3                  Fuel: ")
	print("################FINE################")
	print("Tunnel scavati:           " .. nTunnel)
